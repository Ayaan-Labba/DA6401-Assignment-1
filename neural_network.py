    ...
    def gradient_descent(self, grads, grads_nag=None):
        for i in range(len(self.weights)):
            if self.optimizer == "sgd":
                ...
                self.v_w[i] = self.beta * self.v_w[i] + (1 - self.beta) * (grads["dw"][i].T ** 2)
                self.v_b[i] = self.beta * self.v_b[i] + (1 - self.beta) * (grads["db"][i].T ** 2)
                self.weights[i] -= self.lr * grads["dw"][i].T / (np.sqrt(self.v_w[i]) + self.epsilon)
